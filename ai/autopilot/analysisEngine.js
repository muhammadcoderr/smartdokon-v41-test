...
function analyzeAutopilotData({
  ...
  loginEvents.forEach((event) => {
    const key = String(event.sellerId || "");
  ...
  payments.forEach((payment) => {
    ...
    if (!sellerPerformanceMap[key]) {
      sellerPerformanceMap[key] = {
    ...
    sellerPerformanceMap[key].revenue += getPaymentRevenue(payment);
    sellerPerformanceMap[key].profit += sumProductProfit(payment.products);
    sellerPerformanceMap[key].salesCount += 1;
  });
  ...
  const dailyActionPlan = [
    ...
    },
    {
      ...
      owner: "Admin / Menejer",
      priority: sellerPerformance.some((item) => item.activityStatus === "idle") ? "medium" : "low",
      task:
        sellerPerformance.find((item) => item.activityStatus === "idle")
          ? `${sellerPerformance.find((item) => item.activityStatus === "idle").name} uchun qisqa sotuv sprint rejasi tuzing.`
  ...
  const recommendations = [
    ...
    },
    {
      ...
      priority: sellerPerformance[0]?.sharePercent >= 35 ? "medium" : "low",
      summary:
        sellerPerformance[0]
          ? `${sellerPerformance[0].name} tushumning ${sellerPerformance[0].sharePercent}% ulushini olib kelmoqda.`
