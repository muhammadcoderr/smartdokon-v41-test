...
const { ProductSchema } = require('../shared/database/models/Product');
const { StockSchema } = require('../shared/database/models/Stock');
const BonusSettings = require('../shared/database/models/BonusSettings');
...
class ClientBot {
    ...
    async handleStart(msg, startParam = '') {
        ...
        if (param.startsWith('BR_')) {
            branchId = param.replace('BR_', '');
        } else {
            referralCode = param;
        }

        try {
            let botUser = await BotUser.findOne({ chatId });
            ...
            if (!botUser) {
                ...
                botUser.isClientBotUser = true;
                if (branchId && mongoose.Types.ObjectId.isValid(branchId)) {
                    botUser.branchId = branchId;
                }
                await botUser.save();
            }

            let branchName = "Smart Do'kon";
            if (botUser.branchId) {
                const br = await Branch.findById(botUser.branchId);
                if (br) branchName = br.name;
            }

            let existingClient = null;
            if (botUser.clientId) {
                const conn = await this.getBranchConnection(botUser.branchId);
                const ClientModel = getModel(conn, "Client", ClientSchema);
                existingClient = await ClientModel.findById(botUser.clientId);
            }

            if (existingClient) {
                await this.showMainMenu(chatId, existingClient.firstname, branchName);
            } else {
                await this.showWelcomeMenu(chatId, firstName, branchName, referralCode);
            }
        } catch (error) {
    ...
    async getBranchConnection(branchId) {
        if (!branchId) return mongoose.connection;
        const branch = await Branch.findById(branchId);
        if (!branch) return mongoose.connection;
        return await getTenantConnection(branch.dbName, branch.mongoUri);
    }

    async showWelcomeMenu(chatId, firstName, branchName, referralCode = '') {
        const keyboard = {
            reply_markup: {
                inline_keyboard: [
                    [{ text: '📝 Ro\'yxatdan o\'tish', callback_data: `register_${referralCode}` }],
                    [{ text: '🔐 Kirish', callback_data: 'login' }],
                    [{ text: '🏢 Filialni tanlash', callback_data: 'select_branch' }]
                ]
            }
        };
        ...
        const message = `Assalomu alaykum, ${safeFirstName}! 👋\n` +
            `<b>${safeBranchName}</b> xizmatiga xush kelibsiz.\n\n` +
            `Botdan foydalanish uchun ro'yxatdan o'ting yoki tizimga kiring.\n\n` +
            `${referralCode ? `🎁 Sizda taklif kodi bor: ${escapeHtml(referralCode)}` : ''}`;

        await this.bot.sendMessage(chatId, message, { parse_mode: 'HTML', ...keyboard });
    }

    async showMainMenu(chatId, firstName, branchName) {
        const keyboard = {
            reply_markup: {
                inline_keyboard: [
                    ...
                    [{ text: '🛍 Buyurtmalarim', callback_data: 'my_orders' }, { text: '💳 Qarzlarim', callback_data: 'my_debts' }],
                    [{ text: "🏢 Filialni o'zgartirish", callback_data: 'select_branch' }],
        ...
        const message = `Xush kelibsiz, <b>${safeFirstName}</b>! 🏪\n` +
            `Joriy filial: <b>${safeBranchName}</b>\n\n` +
    ...
    async handleCallbackQuery(callbackQuery) {
        ...
        try {
            ...
            if (data === 'select_branch') {
            ...
            } else if (data.startsWith('set_br_')) {
                const branchId = data.replace('set_br_', '');
                await BotUser.updateOne({ chatId }, { branchId });
                const br = await Branch.findById(branchId);
                const safeBranchName = escapeHtml(br?.name || "Smart Do'kon");
                await this.bot.sendMessage(chatId, `✅ Filial tanlandi: <b>${safeBranchName}</b>`, { parse_mode: 'HTML' });
                
                let clientName = 'Foydalanuvchi';
                if (user.clientId) {
                ...
                await this.showMainMenu(chatId, clientName, br?.name || "Smart Do'kon");
                ...
                const br = await Branch.findById(user.branchId);
                const client = user?.clientId ? await this.getClientFromBranch(user.branchId, user.clientId) : null;
                await this.showMainMenu(chatId, client?.firstname || 'Foydalanuvchi', br?.name || "Smart Do'kon");
            }
        } catch (error) {
    ...
    async getClientFromBranch(branchId, clientId) {
        const conn = await this.getBranchConnection(branchId);
        const ClientModel = getModel(conn, "Client", ClientSchema);
    ...
    async showBranchSelection(chatId) {
        ...
        const keyboard = {
            reply_markup: {
                inline_keyboard: branches.map(br => ([{ text: br.name, callback_data: `set_br_${br._id}` }]))
            }
        };
        await this.bot.sendMessage(chatId, "🏢 <b>Marhamat, o'zingizga yaqin filialni tanlang:</b>", { parse_mode: 'HTML', ...keyboard });
    }
    ...
    async handleRegistrationPhone(chatId, phone, userState) {
        ...
        const user = await BotUser.findOne({ chatId });
        const conn = await this.getBranchConnection(user?.branchId);
        const ClientModel = getModel(conn, "Client", ClientSchema);

        const existing = await ClientModel.findOne({ phone: phoneNumber });
        if (existing) {
    ...
    async handleRegistrationLogin(chatId, login, userState) {
        ...
        const existing = await ClientModel.findOne({ login });
        if (existing) {
    ...
    async handleRegistrationAddress(chatId, address, userState) {
        try {
            ...
            const conn = await this.getBranchConnection(user?.branchId);
            const ClientModel = getModel(conn, "Client", ClientSchema);
            const BonusSettingsModel = getModel(mongoose.connection, "BonusSettings", BonusSettings.schema);
            ...
            if (userState.referralCode) {
                const referrer = await ClientModel.findOne({ referralCode: userState.referralCode });
                if (referrer) {
                    const bonusSettings = await BonusSettingsModel.findOne();
                    ...
                    try {
                        const referrerUser = await BotUser.findOne({ clientId: referrer._id });
            ...
            await BotUser.updateOne({ chatId }, { clientId: newClient._id, role: 'client' });
            this.userStates.delete(chatId);

            await this.bot.sendMessage(chatId, `✅ Ro'yxatdan o'tish muvaffaqiyatli yakunlandi!\nLogin: ${userState.login}\nBonus: ${newClient.bonus} so'm`);
            this.showMainMenu(chatId, userState.firstname, "Smart Do'kon");
        } catch (e) {
            console.error(e);
    ...
    async handleLoginPassword(chatId, password, userState) {
        try {
            ...
            const client = await ClientModel.findOne({ login: userState.login });
            ...
            await BotUser.updateOne({ chatId }, { clientId: client._id, role: 'client' });
            this.userStates.delete(chatId);

            await this.bot.sendMessage(chatId, `✅ Xush kelibsiz, ${client.firstname}!`);
            const br = await Branch.findById(user?.branchId);
            await this.showMainMenu(chatId, client.firstname, br?.name || "Smart Do'kon");
        } catch (e) {
            console.error(e);
            this.bot.sendMessage(chatId, 'Xatolik yuz berdi.');
        }
    ...
    async handleSetBirthday(chatId, dateStr) {
        try {
            ...
            await ClientModel.findByIdAndUpdate(user.clientId, { birthday: date });
            this.userStates.delete(chatId);
    ...
    async showClientInfo(chatId) {
        try {
            const user = await BotUser.findOne({ chatId: Number(chatId) });
            if (!user || !user.clientId) {
    ...
    async showClientOrders(chatId, page = 1) {
        try {
            ...
            const conn = await this.getBranchConnection(user.branchId);
            const PaymentModel = getModel(conn, "Payment", PaymentSchema);

            const limit = 5;
            const skip = (page - 1) * limit;

            const totalOrders = await PaymentModel.countDocuments({ clientId: user.clientId.toString() });
            const totalPages = Math.ceil(totalOrders / limit);

            const recentOrders = await PaymentModel.find({ clientId: user.clientId.toString() })
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(limit);
    ...
    async showHelp(chatId) {
        ...
        const message = `📞 Yordam va aloqa:\n\n🏪 Do'kon: ${br?.name || "Smart Do'kon"}\n📍 Manzil: ${br?.address || "Kiritilmagan"}\n📞 Tel: ${br?.phone || "Kiritilmagan"}\n\nQanday yordam bera olamiz?`;
        await this.bot.sendMessage(chatId, message, keyboard);
    }
