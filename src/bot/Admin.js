...
const BotUser = require('../shared/database/models/Bot/BotUser'); // Foydalanuvchi modeli
...
function initAdminPanel(bot) {
  ...
  async function showPage(chatId, users, page, totalPages) {
    clearPreviousMessages(chatId); // Eski xabarlarni o'chirish
...
Rol: ${user.role}
    ...
    if (data.startsWith('makeadmin_')) {
      ...
      const userId = data.split('_')[1];
      await BotUser.findByIdAndUpdate(userId, { role: 'admin' });
  ...
  bot.onText(/\/addadmin (.+)/, async (msg, match) => {
    ...
    const user = await BotUser.findOne({ chatId: userId });
    if (!user) {
      return bot.sendMessage(chatId, 'Foydalanuvchi topilmadi.');
    }

    await BotUser.findOneAndUpdate({ chatId: userId }, { role: 'admin' });
